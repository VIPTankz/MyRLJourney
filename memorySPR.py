...
class ReplayMemory():
  def __init__(self, capacity, n, discount, device, K):
    self.device = device
  ...
  def _get_samples_from_segments(self, batch_size, p_total):
    ...
    # This is not finished - _get_transitions is doing some stuff to do with N
    # this might need to be replaced by K?
    # Ideally I just want to know where the terminals are and I can blank them at the final stage before the
    # cosine similarity by setting them equal to each other

    # The code below is definitely wrong, the self.n in _get_transitions is not what I want here
    # I think there's a way I can do it by changing the first line of that method to self.K+1, and repeating it 5 times
