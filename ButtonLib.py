...
class Button():
    def __init__(self, x, y, width, height, colour, colourH, border=False, borderC=(0, 0, 0),
                 text="", font=0, size=30, textColour=(0, 0, 0), stripe=None, stripeH=None,
                 stripeThickness=None):  # Initialise all of the variables
    ...
    def hovering(self, pos):  # Method to check if mouse is over the button
        if (pos[0] > self.x and pos[0] < (self.width + self.x) and pos[1] > self.y and pos[1] < (
                self.height + self.y)):  # checks if the mouse is over the button
    ...
    def create(self, screen):  # Method to blit the button onto the screen
        if self.border:  # Checks if the button has a border
            pygame.draw.rect(screen, self.borderC, (self.x - 3, self.y - 3, self.width + 6, self.height + 6),
                             0)  # creates a rectangle larger than the button
        ...
        if self.stripe != None:
            pygame.draw.rect(screen, self.stripeC,
                             (self.x, self.y + (self.height / 2) - int(self.stripeThickness / 2), self.width,
                              self.stripeThickness), 0)
        ...
        if self.text != "":  # checks if there is text for the button
            ...
            screen.blit(text, (self.x + (self.width / 2 - text.get_width() / 2), self.y + (
                        self.height / 2 - text.get_height() / 2)))  # draws the text in the centre of the button
    ...
    def setCorners(self, co1, co2, co3, co4=(0, 0)):  # takes four coordinates to set the values of the button
        if co2[0] > co1[0] and co2[1] < co3[1] and co2[1] == co1[1] and co2[0] > co3[0] and co3[0] == co1[0] and co3[
            1] > co1[1]:  # checks if the coordinates create a valid rectangle
            self.x = co1[0]
